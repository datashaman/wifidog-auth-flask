...
def update_transaction(id, response):
    ...
    current_app.logger.error('update transaction', extra={'processor': processor})
    processor_module = get_processor(id)
    merchant_reference = processor_module.get_merchant_reference(response)
    current_app.logger.error('update transaction', extra={'merchant_reference': merchant_reference})
    order = Order.query.get_or_404(merchant_reference)
    current_app.logger.error('update transaction', extra={'order': order})
    processor_reference = processor_module.get_processor_reference(response)
    current_app.logger.error('update transaction', extra={'processor_reference': processor_reference})
    transaction = processor.transactions.filter_by(processor_reference=processor_reference).first()
    current_app.logger.error('update transaction', extra={'transaction': transaction})
